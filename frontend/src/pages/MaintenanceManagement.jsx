...
function MaintenanceManagement() {
  ...
  const dashboardCards = state.dashboardStats ? [
    ...
    { 
      title: "Assigned", 
      count: state.dashboardStats.overview?.assigned || 0, 
    ...
    { title: "Assigned", count: 0, subtitle: "Vehicles assigned", icon: "📅" },
  ...
  const serviceColumns = [
    ...
    { key: "status", label: "Status", options: ["", "Assigned", "Completed"] },
  ];
  ...
  const repairColumns = [
    ...
    { key: "requestDate", label: "Request Date for the Maintenance" },
    {
      ...
      render: (row, onAction) => {
        if (row.approvalStatus === 'Approved') {
          return <span className="status-tick approved">✓ Approved</span>;
        }
        if (row.approvalStatus === 'Rejected') {
          return <span className="status-tick rejected">✓ Rejected</span>;
        }
        return (
    ...
    { key: "companyName", label: "Company Name" },
    { key: "shiftDate", label: "Shift Date of the Repair" },
    { key: "completeDate", label: "Complete Date of the Repair" },
    { key: "cost", label: "Cost (Rs)" },
    { key: "status", label: "Status", options: ["", "Assigned", "Completed"] },
  ];

  // Once a repair is Rejected, nothing else on that row can be filled in.
  // Once a repair is Approved, Company Name / Shift Date / Complete Date /
  // Cost / Status stay locked until the approval process (Pending Repair
  // Approvals table) has finished - i.e. processCompleted is true.
  const repairColumnLock = (col, rowData) => {
    if (rowData.approvalStatus === 'Rejected') return true;
    const lockedUntilProcessComplete = ['companyName', 'shiftDate', 'completeDate', 'cost', 'status'];
    if (lockedUntilProcessComplete.includes(col.key)) {
      return !(rowData.approvalStatus === 'Approved' && rowData.processCompleted);
    }
    return false;
  };
  ...
  const handleRepairAction = async (action, row) => {
    if (action === "approve") {
      try {
        await approveRepair(row._id, '');
        // Jump straight to the approval-process table for this repair.
