...
exports.getAuditStats = async (req, res) => {
  try {
    // Total completed
    const totalCompletedServices = await Service.countDocuments({
      status: 'Completed',
      completeDate: { $exists: true, $ne: null }
    });

    const totalCompletedRepairs = await Repair.countDocuments({
      status: 'Completed',
      completeDate: { $exists: true, $ne: null }
    });
    ...
    const completedServicesThisMonth = await Service.countDocuments({
      status: 'Completed',
      completeDate: { $gte: startOfMonth }
    });

    const completedRepairsThisMonth = await Repair.countDocuments({
      status: 'Completed',
      completeDate: { $gte: startOfMonth }
    });

    // Total cost
    const serviceCostAgg = await Service.aggregate([
      {
        $match: {
          status: 'Completed',
          completeDate: { $exists: true, $ne: null }
    ...
    const repairCostAgg = await Repair.aggregate([
      {
        $match: {
          status: 'Completed',
          completeDate: { $exists: true, $ne: null }
    ...
    const totalServiceCost = serviceCostAgg[0]?.totalCost || 0;
    const totalRepairCost = repairCostAgg[0]?.totalCost || 0;

    // Most used companies
    const topServiceCompanies = await Service.aggregate([
      {
        $match: {
          status: 'Completed',
          completeDate: { $exists: true, $ne: null }
    ...
    const topRepairCompanies = await Repair.aggregate([
      {
        $match: {
          status: 'Completed',
          completeDate: { $exists: true, $ne: null }
    ...
    res.status(200).json({
      ...
      data: {
        totals: {
          services: totalCompletedServices,
          repairs: totalCompletedRepairs,
          combined: totalCompletedServices + totalCompletedRepairs
        },
        thisMonth: {
          services: completedServicesThisMonth,
          repairs: completedRepairsThisMonth,
          combined: completedServicesThisMonth + completedRepairsThisMonth
        },
        costs: {
          services: totalServiceCost,
          repairs: totalRepairCost,
          total: totalServiceCost + totalRepairCost
        },
        topCompanies: {
          services: topServiceCompanies,
          repairs: topRepairCompanies
        }
      }
    });
  } catch (error) {
